...
+ (id)loadingViewInView:(UIView *)aSuperview{
	...
	return [self loadingViewInView:aSuperview andTitle:nil];
}


+ (id)loadingViewInView:(UIView *)aSuperview andTitle:(NSString*)title
{
	CGRect superViewRect = [aSuperview frame];
    
    UIScrollView* scrollView = (UIScrollView*)aSuperview;
	LoadingView *loadingView = nil;
    
    if([scrollView respondsToSelector:@selector(contentOffset)]){
    
        loadingView = [[[LoadingView alloc] initWithFrame:
				CGRectMake((superViewRect.size.width-280)/2, (superViewRect.size.height + scrollView.contentOffset.y)/2, 280.f, 200.f)] autorelease];
    }else{
        loadingView = [[[LoadingView alloc] initWithFrame:
                        CGRectMake((superViewRect.size.width-280)/2, (superViewRect.size.height - 200)/2, 280.f, 200.f)] autorelease];
    }
